...
@router.get("/preset/library")
async def preset_library() -> dict:
    """List every preset generated by the latest swarm run. Powers the
    merchant dashboard gallery — returns the full HTML+CSS + voter count."""
    db = supa()
    rows = (
        db.table("preset_library")
        .select(
            "id,display_name,description,change_summary,generated_html,"
            "generated_css,voter_twin_ids,run_id"
        )
        .order("display_name")
        .execute()
        .data
